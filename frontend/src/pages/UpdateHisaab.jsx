...
const UpdateHisaab = () => {
    const { updateHisaab, isHisaabUpdating, selectedHisaab } = useHisaabStore();
    const [hisaabData, setHisaabData] = useState({
      title: selectedHisaab.title,
      description: selectedHisaab.description,
      isEncrypted: selectedHisaab.isEncrypted,
      passcode: selectedHisaab.passcode,
    });
    ...
    const handleUpdateHisaab = async (e) => {
      e.preventDefault();
      const valid = validateHisaab(hisaabData);
      if (valid === true) await updateHisaab(selectedHisaab._id, hisaabData);
      setHisaabData({
        title: "",
        description: "",
        isEncrypted: false,
    ...
    return (
      <>
        <div className="w-1/2 pl-10 pt-10">
          <form onSubmit={handleUpdateHisaab} className="flex flex-col gap-4">
